...
jest.mock("react-mdl", () => ({
    Checkbox: jest.fn().mockImplementation(({ children }) => {
        return children;
    }),
    Card: jest.fn().mockImplementation(({ children }) => {
        return children;
    }),
    Menu: jest.fn().mockImplementation(({ children }) => {
        return children;
    }),
    MenuItem: jest.fn().mockImplementation(({ children }) => {
        return children;
    })
}));
...
test("changing projects renders only toggles from that project", () => {
    ...
    expect(select.contains(<option value="default">Default</option>)).toBe(
        true
    );
    expect(select.contains(<option value="myProject">MyProject</option>)).toBe(
        true
    );
