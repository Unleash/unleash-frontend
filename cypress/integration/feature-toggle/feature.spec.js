...
describe('feature toggle', () => {
    ...
    it('Can add two variant to the feature', () => {
        ...
        cy.intercept('PATCH', `/api/admin/projects/default/features/${featureToggleName}`, req => {
    ...
    it('Can set weight to fixed value for one of the variants', () => {
        ...
        cy.get('[data-test=VARIANT_NAME_INPUT]').children().find('input').should('have.attr', 'disabled');
        cy.get('[data-test=VARIANT_WEIGHT_TYPE]').children().find('input').check();
        ...
        cy.get('[data-test=VARIANT_WEIGHT]').first().should('have.text', '15 %');
    });
